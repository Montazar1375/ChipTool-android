...
import chip.devicecontroller.ClusterIDMapping.Actions
          ...
          val scanCallback =
            object : ScanCallback() {
              override fun onScanResult(callbackType: Int, result: ScanResult) {
                ...
                if (result.scanRecord?.serviceData?.containsKey(ParcelUuid(UUID.fromString(CHIP_UUID))) == true){
                  Log.i(TAG, "Bluetooth Device Scanned Addr: ${device.address}, Name: ${device.name},M: ${ParcelUuid(UUID.fromString(CHIP_UUID))} UUID: ${result.scanRecord?.serviceData} ")
                }


                if (device.name == "UASCENT"){
...
//            .setServiceUuid(ParcelUuid(UUID.fromString(CHIP_UUID)))
              .build()
